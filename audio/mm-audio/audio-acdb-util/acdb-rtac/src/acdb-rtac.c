...
{
	struct afe_st_set_gain_param_payload  params = {0};
...
void pack_afe_sidetone_set_rtac_enable_data(ACPH_SET_AFE_SIDETONE_GAIN_req *rtac_info,
		uint32_t in_buf_size, uint32_t out_buf_size)
{
	struct afe_st_set_enable_param_payload  params = {0};
...
static int32_t get_afe_sidetone_rtac(struct ACPH_GET_AFE_SIDETONE_GAIN_req *rtac_info,
		uint32_t in_buf_size, uint32_t *rtac_data, uint32_t out_buf_size,
		uint32_t *bytes_filled)
{
	int32_t ret = 0;
	struct afe_sidetone_get_param_resp         sidetone_rsp = {0};
